...
export default function LocationTab({ totalTabs }: { totalTabs: number }) {
  return (
    <FolderSection title="Location" index={10} color="bg-background" tab={10} totalTabs={totalTabs}>
      <MouseSphere />
      <div className="flex-1 flex flex-col md:flex-row gap-0 min-h-0">
        <div className="flex-1 md:w-3/4 md:flex-none overflow-hidden min-h-0">
          <MapEmbed />
        </div>
        <div className="md:w-1/4 flex flex-col justify-center gap-3 md:gap-5 px-5 md:px-8 py-4 md:py-0 border-t md:border-t-0 md:border-l border-foreground/10">
