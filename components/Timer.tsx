...
export default function Timer({ className }: TimerProps) {
  ...
  return (
    <div className="flex items-center justify-center flex-1 w-full px-6">
      {/* Mobile: stacked, centered */}
      <div className="flex md:hidden flex-col items-center gap-6 w-full">
        {units.map(({ value, label }) => (
          <div key={label} className="flex flex-col items-center gap-1">
            <div
              className={`glitch text-[18vw] font-bold leading-none ${className || ""}`}
            >
        ...
        {units.map(({ value, label }, i) => (
          <React.Fragment key={label}>
            <div className="flex flex-col items-center gap-2">
              <div
                className={`glitch text-[8vw] font-bold leading-none ${className || ""}`}
              >
                <GlitchText text={String(value).padStart(2, "0")} />
              </div>
            ...
            {i < units.length - 1 && (
              <span
                className={`glitch text-[6vw] font-bold text-white leading-none mt-[0.5vw] ${className || ""}`}
              >
                <GlitchText text=":" />
              </span>
            )}
